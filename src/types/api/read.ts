...
import { FetchNodeUnchecked } from "./fetch_node";
...
interface ReadRequestData<Entity extends EntityRestriction<Entity>, Relations extends FetchNodeUnchecked<Entity>> {
    type: Ctor<Entity>;
    relations: Any.Cast<Relations, NoExtraProperties<FetchNodeUnchecked<Entity>, Relations>>;
}
...
export type ReadRequest<Entity extends EntityRestriction<Entity>, Relations extends FetchNodeUnchecked<Entity>> = ReadRequestData<Entity, Relations> & ReadRequestIds<Entity> & ReadRequestTemporal;
...
export interface DatabaseRead<Universe extends UniverseRestriction<Universe>> {
    <T extends { [prop: string]: ReadRequest<UniverseElement<Universe>, any> }>(
        readMany: { [P in keyof T]: Any.Cast<T[P], ReadRequest<RequestEntity<T[P]>, RequestRelations<T[P]>>> }
